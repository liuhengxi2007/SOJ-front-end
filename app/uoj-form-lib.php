<?php
	class UOJForm {
		...
		private $vdata = array();
		private $main_html = '';
		public $max_post_size = 52428800; // 50M
...
EOD;
	...
	function newSubmissionForm($form_name, $requirement, $zip_file_name_gen, $handle, $allow_estimate, $estimate_config) {
		$form = new UOJForm($form_name);
		...
		if ($allow_estimate) {
			$form->addInput('estimate', 'text', UOJLocale::get('problems::estimate result'), $estimate_config['default'],
				function($result) use($estimate_config) {
					if (!validateUInt($result) || (int)$result > $estimate_config['max']) {
						return "预估结果需为 0 ~ {$estimate_config['max']} 之间的非负整数";
					}
					return '';
				},
				null
		...
		$form->handle = function(&$vdata) use($form_name, $requirement, $zip_file_name_gen, $handle) {
			...
			if (!is_null($estimate = $_POST['estimate']))
				$estimate = (int)$estimate;

			$handle($zip_file_name, $content, $tot_size, $estimate);
		};
		return $form;
	}
	function newZipSubmissionForm($form_name, $requirement, $zip_file_name_gen, $handle, $allow_estimate, $estimate_config) {
		$form = new UOJForm($form_name);
