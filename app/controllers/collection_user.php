<?php
	...
	$cond = "exists (select 1 from collection_user_tags where collection_user_tags.collection_id = collection_user.id and collection_user_tags.tag = '{$cur_tab}') and owner='{$owner}'";
	...
	$username = Auth::id();
	$pag_config['table_name'] = "collection_user";
...
<?php if(isProblemCreator(Auth::user()) || Auth::id() == $owner):?>
	<div class="col-xs-6 col-sm-6">
		<ul class="nav nav-pills" role="tablist"><li><a href="/collection/<?= $owner ?>/manage"><span class="glyphicon glyphicon-cog"></span>  <?= UOJLocale::get('manage') ?></a></li></ul>
	</div>
<?php endif?>
</div>
