...
function getUserLink(username, rating) {
	...
	return '<a class="uoj-username' + (rating >= 2600 ? ' legendary' : '') + '" href="/user/profile/' + username + '" style="color: ' + getColOfRating(rating) + '">' + text + '</a>';
}
...
function getGroupLink(groupname, rating) {
	...
	var text = groupname;
	return '<a class="uoj-username' + (rating >= 2600 ? ' legendary' : '') + '" href="/group/' + groupname + '" style="color: ' + getColOfRating(rating) + '">' + text + '</a>';
}
...
$.fn.uoj_problem_tag = function() {
	return this.each(function() {
		$(this).attr('href', '/problems?tag=' + encodeURIComponent($(this).text()));
	});
}
...
function require_codemirror(config, callback) {
	if ($('link[href="/js/codemirror/lib/codemirror.css' + '"]').length == 0) {
		$('<link type="text/css" rel="stylesheet" href="/js/codemirror/lib/codemirror.css' + '" />').appendTo('head');
	}
	$LAB.script('/js/codemirror/lib/codemirror.js')
		.wait()
		.script('/js/codemirror/addon/mode/overlay.js')
		.script('/js/codemirror/addon/selection/active-line.js')
		.wait(callback)
}
...
function require_codemirror_mode(mode, callback) {
	...
	if (name !== 'none') {
		$LAB.script('/js/codemirror/mode/' + name + '/' + name + '.js')
			.wait(callback);
	} else {
...
function showStandingsACM(wa_penalty, getLink, doNotShowLinks) {
	var best = [], cur, col, ac = Array(problems.length).fill(0), sub = Array(problems.length).fill(0);
...
function showStandingsSPC(wa_penalty, getLink, doNotShowLinks) {
	var best = [], cur, col, full_score = 0, ac = Array(problems.length).fill(0), sub = Array(problems.length).fill(0);
	if (wa_penalty === undefined) {
		wa_penalty = 1200;
	}
	if (getLink === undefined) {
