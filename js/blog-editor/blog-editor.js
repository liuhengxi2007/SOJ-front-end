function blog_editor_init(name, editor_config) {
	...
	var help_btn = $('<button type="button" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-globe"></span></button>');
	...
	help_btn.tooltip({ container: 'body', title: '帮助 (Ctrl-H)' });
	
	var all_btn = [save_btn, preview_btn, bold_btn, italic_btn, help_btn];
	...
	help_btn.click(function() {
		if (editor_config.type == 'blog') {
			window.open("/blog/65", "_blank"); 
		} else if (editor_config.type == 'slide') {
			window.open("/blog/52", "_blank"); 
		}
	});
